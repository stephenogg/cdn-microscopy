...
## Overview
Designed for imaging fixed samples in fluorescence mode, this LSM800 is equipped with 4 lasers, 3 detectors (one Airyscan), and a variety of lenses for imaging samples with varying length scales, from single cells to fish. Samples for this system should be mounted on slides under coverslips, or in petri dishes. This system is configured to accommodate blue, green, red and far red fluorophores either individually, or in combination. Acquisition software is Zeiss' Zen version 2.6. The software enables collection of X/Y planes from 128 X 128 pixels up to 4096 X 4096 pixels, Z-stacks, time-lapse, and tiles. The Airyscan detector **can** improve the resolution of your image up to ~1.7X.
The confocal is attached to an Axio Examiner.Z1 -- a fixed stage microscope stand. On this stand, focusing moves the lens along the Z-axis with respect to the sample. This enables imaging of samples with vastly different length scales --- e.g. live zebrafish or cells under coverslips. On this system, the objective turret rotates manually, and the fluorescence filter turret rotates manually. When viewing fluorescence through the eyepieces, you can select from 3 filter cubes: Fs38, Fs43, Fs49.

|  Turret Position | Emission Colour| Filter Name | Excitation | Dichroic | Emission |
|:-----------:|:----------:|:--------:|:---------:|:--------:|:---------:|
|1|Green|Fs38|470/40|FT495|525/50|
|2|Red|Fs43|545/25|FT570|605/70|
|3|Blue|Fs49|G365|FT395|445/50|

{{< columns >}}
- Lasers: <span style="background-color:purple;color: yellowgreen;">&nbsp;405nm&nbsp;</span>, <span style="background-color:cyan;color: orangered;">&nbsp;488nm&nbsp;</span>, <span style="background-color:greenyellow;color: violet;">&nbsp;561nm&nbsp;</span>, <span style="background-color:darkred;color: #00cec8;">&nbsp;642nm&nbsp;</span>
...
- LSM800 scanner with two variable spectral dichroic mirrors to direct distinct wavelengts of the emission fluorescence into three separate detectors.
...
<li>In the electronics cabinet, switch on the "System/PC" switch on the PSU module &mdash; <span style="background-color:orange;color:black; border-radius:50px;">&nbsp;1&nbsp;</span></li>
<li>Switch on the "Components" switch on the PSU module &mdash; <span style="background-color:orange;color:black;border-radius:50px;">&nbsp;2&nbsp;</span></li>
<li>Turn the Laser Key &mdash; from vertical to horizontal on the LM module &mdash; <span style="background-color:orange;color:black;border-radius:50px;">&nbsp;3&nbsp;</span></li>
<ul>
...
This confocal microscope uses an Axio Examiner.Z1 stand. It has a fixed position stage, focusing moves the lens up or down. It has a manually rotating objective lens turret and fluorescence filter turret. To accommodate samples of different sizes the entire lens moves from a "load" position, far away from your sample, to an "imaging" position, with the front lens near your sample. Moving from the load to the imaging positions is achieved by rotating the big black knob on the front of the microscope, near the lens turret. ---
<span style="background-color:orange;color:black;border-radius:50px;">&nbsp;6&nbsp;</span>
{{< columns >}}
...
1. If required, place immersion media on top of your sample.
1. Lower the lens into the imaging position using the black rotating knob (see previous section).

{{< alert  >}}
Do this slooooowly. The lens may lower and crush your sample, depending on the last users' settings.{{< /alert >}}
1. You may need to adjust the height of the lens for your sample type. 
...
1. Using the "Locate" tab in the software, activate the contrast method you will use to focus. This is usually a transmitted light mode or DAPI fluorescence. Press the appropriate button to turn on the light. --- <span style="background-color:orange;color:ghostwhite;border-radius:50px;">&nbsp;7&nbsp;</span>
	...
	- OFF: Off
1. Select the appropriate filterset for viewing the fluorophore you want to image. Remember, this is a manual microscope.
...
10. Use the joystick on the stage controller to find an appropriate field of view.
10. You are done with the microscope. Move the filter reflector turret to an open position (position 4 or 5), so that all the emitted light will go to the scanhead.
{{< column >}}
...
The easiest way to get started is to tell the confocal which set of fluorophores you have in your sample and let the system configure the optical components.
...
{{< figure src=drawing.svg
					default=true
					caption="Acquisition Tab"
					class="center" >}}
{{< column >}}
{{< figure src=smartSetup.svg
					default=true
					caption="Smart Setup Pane"
					class="center" >}}
{{< endcolumns >}}
...
1. This opens the "Add a dye or contrasting method" pane.
{{< columns>}}
{{< figure src=dyeList.PNG
					default=true
					class="center"
					height="50px" >}}
{{< column >}}
{{< figure src=smartSetupBGR.PNG
					default=true
					class="center" >}}
{{< endcolumns >}}
1. From this window, select the set of dyes in your sample. This will populate the smart setup pane with your dye choices.
...
1. The smart setup pane will now be populated with your dye choices and you must make several decisions about how you would like to image this set of dyes.
	...
	1. Choose either LSM Confocal or Airyscan SR -- Regular Confocal Vs. Super Resolution
	...
	1. Tip --- Always choose "Best Signal" when you have fixed samples. 
</details>
<details>
